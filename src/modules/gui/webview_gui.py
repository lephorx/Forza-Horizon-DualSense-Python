...
def _frame_pct(frame) -> int:
    ...
    if mode in (M_RIGID_ZONES, M_VIBRATE_ZONES):
        return 220  # wall active — show near-full
    return 0
...
class API:
    ...
    # -- tray restore (called from JS "Show" button) --
...
class WebViewGUI:
    ...
    def _on_closing(self):
        """Hide to tray/dock instead of closing."""
        if self._tearing_down:
            return True  # allow actual close when quitting
        if self._window:
            self._window.hide()
        self._hidden = True
    ...
    def _start_session(self):
        ...
        try:
            ...
            # Wrap set() to track last sent frames for live bars
    ...
    def _status_dict(self) -> dict:
        ...
        now = time.monotonic()
        if self._last_packet_t:
            pkt_ago = round(now - self._last_packet_t, 1)
        else:
            pkt_ago = None
